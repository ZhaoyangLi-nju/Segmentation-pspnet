...
class CityScapes(torch.utils.data.Dataset):
    def __init__(self,cfg=None,transform=None,phase_train=True,data_dir=None):
        ...
        self.class_weights=[0.3045383480249677, 1.2862127312658735, 0.4509888876996228, 3.8156945930092214,
        3.525278401818165, 3.1482608323481944, 4.579224481584843, 3.969406346608758, 0.6063928185273372,
        3.2164844089526534, 1.7109233716903072, 3.1563320141579503, 4.733397232867321, 1.1610673599796504,
        4.460042610251128, 4.523705196392834, 4.528288297518183, 4.814776939659858, 4.19246318335068]
        self.cfg=cfg
        self.ignore_label = 255
        self.transform=transform
        if phase_train:
        ...
        for train_dir in file_dir:
            ...
            for file_name in file_names:
                ...
                # example["image"] = Image.open(img_path).convert('RGB')#add image into memory
                # image_path_npy=img_path.split(".")[0]+".npy"
                # image=np.array(example["image"])
                # np.save(image_path_npy,image)
                # print("save:",image_path_npy)
                # image = np.load(img_path)#add label into memory
                # example["image"] = Image.fromarray(image.astype(np.uint8),mode='RGB')
                
                # label = np.load(label_img_path)
                # label_copy = label.copy()
                # for k, v in self.id_to_trainid.items():
                #     label_copy[label == k] = v 
                # example["label"] = Image.fromarray(label_copy.astype(np.uint8))#add label into memory

                # if self.cfg.NO_TRANS==False:
                #     seg=np.load(seg_path_npy)
                    # seg_path_npy = label_img_path.split("_gtFine_labelIds.npy")[0]+"_seg.npy"
                    # print(seg_path_npy)
                    # np.save(seg_path_npy,seg.astype(np.uint8))
                    # example["seg"] = Image.fromarray(seg.astype(np.uint8), mode='RGB')
                # print(count)
                # count+=1
...
class Resize(transforms.Resize):
    ...
    def __call__(self, sample):
        ...
        for key in sample.keys():
            if key == 'label':
                sample['label'] = F.resize(sample['label'], (h, w), interpolation=Image.NEAREST)
                continue
            sample[key] = F.resize(sample[key], (h, w))
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, sample):
        if random.random() < 0.5:
            sample['image'] =  sample['image'].filter(ImageFilter.GaussianBlur(radius=self.radius))
        return sample
